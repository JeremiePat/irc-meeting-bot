...
var COMMANDS = {
  help  : {
    type: 'help',
    help: 'Needs some help? Just call me with HELP to get the list of all ' +
          'possible commands, or HELP <command> to get help about a given ' +
          'command.'
  },
  start : {
    type: 'begin',
...
var SPEECH = {
  ...
  format    : ['Minutes have been formatted ({{format}}).'],
  help      : ['{{help}}'],
...
Bot.prototype.registerCommand = function registerCommand(command, name) {
  ...
  try {
    ...
    loggy.error('Unable to register command:', name, e);
  }
};
...
Bot.prototype.onMessage = function (channelName, nick, msg) {
  ...
  if (!channel || !msg.trim().length) { return; }
  ...
  if (!hasCommand) {
    channel.logger.push(this.l10n.getTime(), nick, msg);
  }
};
...
Bot.prototype.onLeave = function (channelName, event) {
  ...
  channel.logger.end(new Formater(
    this.output,
    this.dir,
    'meeting.' + this.l10n.getMoment('YYYYMMDD')
  ));
};
